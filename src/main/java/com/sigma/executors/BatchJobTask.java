...
//	}
      public BatchJobTask(String workSql, JdbcTemplate jdbcTemplate, 
  		...
  		this.gasprice=gasPrice;
  		this.nonceapiurl=nonceApiUrl;
  		this.algorandMnemonic = algorandMnemonic;
  		this.algorandPublic = algorandPublic;
  		this.algorandNetworkUrl = algorandNetworkUrl;
  		this.hederaPublic = hederaPublic;
  		this.hederaPrivate = hederaPrivate;
  		this.hederaContractId = hederaContractId;
  		this.networkConfig = networkConfig;
  		this.hederaNetworkName = hederaNetworkName;
  	}
...
//			  	LOGGER.info("Thread {"+ Thread.currentThread()+"} printing started privateNetwork2" + privateNetwork2);
			  	...
			  	for(SigmaDocument documentO : pendingDocumentsBySQL) {
			  		PolygonEdgeUtil polygonEdgeUtil = new PolygonEdgeUtil();
			  		JSONObject nftInfo = polygonEdgeUtil.mintNftForDocument(documentO, sigmaDocFieldConfigList, infuraurl, contractaddress, privatekey, chainid, gasprice,nonceapiurl, algorandMnemonic, algorandPublic, algorandNetworkUrl, hederaPublic, hederaPrivate, hederaContractId, networkConfig, hederaNetworkName);
			  		documentO.setUuid(nftInfo.optString("uuid"));
			  		documentO.setTxnHash(nftInfo.optString("txnHash"));
			  		documentO.setNftCreationStatus(1);
			  		LOGGER.info("Thread {"+ Thread.currentThread()+"} created NFT for doc id =>  "+documentO.getSigmaId()+
			  				", uuid => "+nftInfo.optString("uuid","Error"));
			  		sigmaDocumentPersistence5.updateImmutableRecord(documentO, jdbcTemplate);
