...
//	}
	...
	public JSONObject mintNftForDocument(
			SigmaDocument documentO, List<SigmaAPIDocConfig> sigmaDocFieldConfigList,String infuraUrl,String contractAddress,String privateKey,int chainId,BigInteger gasPrice,String nonceApiUrl, String algorandMnemonic, String algorandPublic, String algorandNetworkUrl, String hederaPublic,String hederaPrivate, String hederaContractId, String networkConfig, String hederaNetworkName) {
		try {
//		String mintNftUrl = privateNetwork2.getSmartContractAccessUrl()+"contracts"+"/"+ privateNetwork2.getSmartContractAddress() +
//				"/mintNFT?kld-from="	+ privateNetwork2.getSmartContractDefaultWalletAddress() +"&kld-sync=true";
		JSONObject nftInfo = new PolygonEdgeUtil().mintNft( 
				documentO, sigmaDocFieldConfigList, infuraUrl, contractAddress, privateKey, chainId, gasPrice, nonceApiUrl, algorandMnemonic, algorandPublic, algorandNetworkUrl, hederaPublic, hederaPrivate, hederaContractId, networkConfig, hederaNetworkName); //v
		return  nftInfo;
		}catch(Exception exception) {
			LOGGER.error("Error PolygonEdgeUtil.mintNftForDocument()", exception);
			return null;
...
//	}
	...
	private JSONObject mintNft(
        ...
        input.put("fVar11", documentO.getMd5Checksum());
	//return mintNft(mintNftUrl, userName, password, input);
        if(networkConfig.toString().equals("algorand"))
        {
        	try {
				return mintNftAlgorand(input, algorandMnemonic, algorandPublic, algorandNetworkUrl);
			} catch (Exception e) {
        ...
        else if(networkConfig.toString().equals("hedera"))
        {
        	try {
				return mintNftHedera(input, hederaPublic, hederaPrivate, hederaContractId, hederaNetworkName);
			} catch (Exception e) {
				System.out.println(e);
				JSONObject dummyResponse = new JSONObject();
				return dummyResponse;
...
//	}
	...
	public void waitForConfirmation(String txID, AlgodClient client) throws Exception {
	...
	public JSONObject mintNftAlgorand(JSONObject input, String algorandMnemonic, String algorandPublic, String algorandNetworkUrl) throws Exception {
		JSONObject environmentResponse = new JSONObject();
	    final String ALGOD_API_ADDR = algorandNetworkUrl;
	    ...
	    final String account_mnemonic = algorandMnemonic;
	    ...
	    try {
	        ...
	        waitForConfirmation(id, client);
	        // Read the transaction
	        PendingTransactionResponse pTrx = client.PendingTransactionInformation(id).execute().body();
	        // Now that the transaction is confirmed we can get the assetID
	        assetID = pTrx.assetIndex;
	        System.out.println("AssetID = " + assetID);
	        printCreatedAsset(acct, assetID, client);
	        printAssetHolding(acct, assetID, client);
	        if (assetID != null) {
	...
	public static JSONObject mintNftHedera(JSONObject input, String hederaPublic, String hederaPrivate, String hederaContractId, String hederaNetworkName) throws Exception {
	    // Replace with your Hedera network and account information
	    String privateKeyStr = hederaPrivate;
	    PrivateKey privateKey = PrivateKey.fromString(privateKeyStr);
	    AccountId operatorId = AccountId.fromString(hederaPublic);
	    ContractId contractId = ContractId.fromString(hederaContractId);
	    System.out.println("input: " + input);
	    // Pre-configured client for test network (testnet)
	    com.hedera.hashgraph.sdk.Client clientHedera = null;
	    if(hederaNetworkName.toString().equals("testnet"))
	    {
	    	clientHedera = Client.forTestnet();
	    }
	    else if(hederaNetworkName.toString().equals("mainnet"))
	    {
	    	clientHedera = Client.forMainnet();
	    }
	    
	    clientHedera.setOperator(operatorId, privateKey);
	    ...
	    transaction.freezeWith(clientHedera);
	    
	    com.hedera.hashgraph.sdk.TransactionResponse hash = null;
	    ...
	    try {
	        ...
	        com.hedera.hashgraph.sdk.TransactionResponse txResponse = transaction.execute(clientHedera);
	        System.out.println("The transaction Response is " + txResponse);
	        // Request the receipt of the transaction
	        TransactionReceipt receipt = txResponse.getReceipt(clientHedera);
	    ...
	    JSONObject environmentResponse = new JSONObject();
	    environmentResponse.put("uuid", input.optString("tokenKey"));
	    environmentResponse.put("txnHash", (hash.transactionId).toString());
	    return environmentResponse;
	}
...
// 
           	 if(hash != null) {
           				 ...
           				 environmentResponse.put("uuid", input.optString("tokenKey"));
           				 environmentResponse.put("txnHash", hash);
           			     return environmentResponse;
           	 }
   		} 
