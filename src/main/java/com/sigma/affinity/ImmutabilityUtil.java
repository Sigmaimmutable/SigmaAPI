...
//	}
	...
	public void fetchAndCreateImmutabilityRecords(JdbcTemplate jdbcTemplate,
			int batchSize, List<SigmaAPIDocConfig> sigmaDocFieldConfigList, String tId,String infuraUrl,String contractAddress,String privateKey,int chainId,BigInteger gasPrice,String nonceApiUrl, String algorandMnemonic, String algorandPublic, String algorandNetworkUrl, String hederaPublic,String hederaPrivate, String hederaContractId, String networkConfig, String hederaNetworkName) throws Exception{
		try {
		Integer pendingNftCount = jdbcTemplate.queryForObject("SELECT COUNT(1) FROM SIGMA_DOCUMENT WHERE NFT_CREATION_STATUS = 0", Integer.class);
		if(pendingNftCount<=0)
			return;
...
//		int noOfBatches = Math.round(pendingNftCount / batchSize);
		...
		for(int counter = 0; counter <= noOfBatches; counter++) {
			Integer startIndex = counter * batchSize;
			String finalSQL = DOCU_FETCH_SQL + startIndex;
			BatchJobTask batchJobTask = new BatchJobTask(finalSQL, jdbcTemplate, sigmaDocFieldConfigList, infuraUrl, contractAddress, privateKey, chainId, gasPrice, nonceApiUrl, algorandMnemonic, algorandPublic, algorandNetworkUrl, hederaPublic, hederaPrivate, hederaContractId, networkConfig, hederaNetworkName);
			tasks.add(batchJobTask);
			if(counter * noOfBatches > pendingNftCount)
				break;
		}
